...
ros::ServiceClient robot1_io_states_client;
...
void save_data()
{
    cv::FileStorage fs("data_screw.yaml", cv::FileStorage::WRITE);
...
void load_data()
{
    cv::FileStorage fs("data_screw.yaml", cv::FileStorage::READ);
...
{
    robot1_io_states_srv.request.fun = fun;
    robot1_io_states_srv.request.pin = pin;
    robot1_io_states_srv.request.state = state;
...
int main(int argc, char *argv[])
{
    ...
    // while (1)
    {
        ...
        for (int current_hole = 1; current_hole >= 0; current_hole--)
        {
        ...
        int setio_pin = 0;
        double setio_state = 1.0;
        robot1_setio(setio_fun,setio_pin,setio_state);
        ROS_INFO("agv leave");
        ros::Duration(2.0).sleep();
